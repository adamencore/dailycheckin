...
export default function EncoreCheckin() {
  ...
  const extractActionItems = useCallback(async () => {
    ...
    const prompt = "You are the executive assistant to the Executive Director of Encore Theatre in St. George, Utah.\n\nRead these daily check-in notes and extract clear, detailed, well-written action items. Write each as a complete professional sentence with enough context that anyone reading it understands what needs to happen and why.\n\nUPCOMING EVENTS:\n" + UPCOMING_EVENTS + "\n\nCHECK-IN NOTES:\n" + allNotes + "\n\nReturn ONLY a JSON array with no markdown:\n[{\"person\":\"Name or dash\",\"role\":\"Role or empty\",\"task\":\"Full detailed action item with context\",\"priority\":\"high|medium|low\",\"category\":\"team|marketing|week|projects|finance|showprep|executive|aspire\"}]\n\nRules:\n- Write complete well-worded tasks with context\n- Only extract real action items\n- high = urgent/this week, medium = important soon, low = when possible\n- For team items set person to team member name; for others use a dash";
  ...
  async function syncToSheets() {
    ...
    try {
      await fetch("/.netlify/functions/claude", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ model: "claude-sonnet-4-20250514", max_tokens: 1000, messages: [{ role: "user", content: "Append to Google Sheets ID \"" + match[1] + "\". Add header if missing: Date|Category|Person|Role|Notes|Action Items. Append: " + JSON.stringify(rows) + " to Sheet1!A:F." }], mcp_servers: [{ type: "url", url: "https://gcal.mcp.claude.com/mcp", name: "google" }] }) });
      setStep(STEPS.DONE);
  ...
  async function sendToSlack() {
    ...
    let blocks = [
      ...
      { type: "divider" },
      { type: "section", text: { type: "mrkdwn", text: "*📝 Today's Notes*" } },
    ];
      ...
      catLabels.forEach(({ key, label }) => {
        ...
        ["high","medium","low"].forEach(p => {
          ...
          pi.forEach(item => {
            ...
            blocks.push({ type: "section", text: { type: "mrkdwn", text: "• " + item.task + (hp ? "\n_" + mentionOrName(item.person) + " · " + item.role + "_" : "") } });
          });
        });
        blocks.push({ type: "divider" });
      });
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0C0C10", color: "#EDE8E0", fontFamily: "'Palatino Linotype','Book Antiqua',Palatino,Georgia,serif", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", padding: "28px", maxWidth: 680, width: "100%", margin: "0 auto", boxSizing: "border-box" }}>
        ...
        {step === STEPS.REVIEW && (
          <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            <div><div style={{ fontSize: 22, fontWeight: "bold", marginBottom: 6 }}>Today's Action Items</div><div style={{ fontSize: 13, color: "#666" }}>{actionItems.length} items extracted from your check-in notes</div></div>
                        ...
                        return (
                          <div key={priority} style={{ marginBottom: 10 }}>
                            ...
                            {pi.map((item, i) => {
                              const hp = item.person && item.person !== "-" && item.person !== "—";
                              return (
                                <div key={i} style={{ background: p.bg, border: "1px solid " + p.border, borderRadius: 10, padding: "14px 16px", marginBottom: 8, display: "flex", gap: 14, alignItems: "flex-start" }}>
                                  ...
                                  <div><div style={{ fontSize: 14, color: "#EDE8E0", lineHeight: 1.6 }}>{item.task}</div>{hp && <div style={{ fontSize: 11, color: "#666", marginTop: 4 }}>{item.person} · {item.role}</div>}</div>
                                </div>
                              );
                            })}
                          </div>
